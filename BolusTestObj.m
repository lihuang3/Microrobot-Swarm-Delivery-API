...
classdef BolusTestObj < handle
   ...
   methods
       %% class constructor
       function this = BolusTestObj(map)
...
% %            end
       ...
       function GlobalSetup(this)
           ...
           this.BranchRange = zeros(size(this.BW,1),size(this.BW,2),length(this.BrchPts0)); 
           for ii = 1:length(this.BrchPts0)
                temp = this.Path(:,5)==1 & this.Path(:,6)== ii;
                BranchRangeID = rangesearch(this.Path(:,1:2),this.Path(temp,1:2),35);
                for jj = 1:length(BranchRangeID)
                    this.BranchRange(this.Path(BranchRangeID{jj},1),this.Path(BranchRangeID{jj},2),ii) = ...
                        this.BranchRange(this.Path(BranchRangeID{jj},1),this.Path(BranchRangeID{jj},2),ii)+1;
                end
           end
...
%                 hRobot_Target.CData = [0 1 0];              
            ...
            switch this.alg
                ...
                case 7 
                    [nstep, RegionDist] = Divide_N_Conquer_Aggregation_V3(this,nstep,totalcost, hRobot, hRobot_Target);
...
%                         totalcost = sum(this.Path(Idx0,4));
       ...
       function [nstep, RegionDist] = Divide_N_Conquer_Aggregation_V3(this,nstep,totalcost, hRobot, hRobot_Target)
            inertial_mode = 1;      % Inertial motion ON/OFF (switch Case 3)
            Next_Branch = [];            
            Next_Region = [];
            Previous_Region = [];
            ...
            while totalcost > this.NumRob*(100+1/3*this.channel_width)
                    ...
                    DistMap = this.BW.*0;
                    for ii = 1:length(this.NumRob)
                       DistMap(this.loc(ii,1),this.loc(ii,2)) = DistMap(this.loc(ii,1),this.loc(ii,2)) +1; 
                    end
                    ...
                    % The region with highest priority
                    iRegionTarget = ii;
                    Priority_Branch = zeros(length(this.RegionID),1);
                    for ii = iRegionTarget:length(this.RegionID)
                        
                        temp = DistMap.*this.BranchRange(:,:,ii);
                        Priority_Branch(ii) = sum(temp(:));
                    end
                    
                    [max_Val,max_ID] = max(Priority_Branch);
                    if max_Val > this.NumRob*2/100
                        iRegionTarget = max_ID;
                    end
